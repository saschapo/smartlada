...
// Persisted user settings (NVS). Effect timing params live in fx::EFFECTS[] and
// are persisted separately later (TODO); this holds the scalar UI state.
namespace config {
...
struct Settings {
  ...
  uint8_t master;         // ONE global brightness for everything (static + fx)
