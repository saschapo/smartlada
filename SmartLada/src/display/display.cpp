...
// Разводка HW-364A (сверено с github.com/peff74/esp8266_OLED_HW-364A)
static constexpr uint8_t OLED_SDA = 14, OLED_SCL = 12, OLED_ADDR = 0x3C;
static constexpr uint8_t BTN_FLASH = 0;  // LOW = нажата
static constexpr uint8_t NUM_PAGES = 3;
static constexpr uint16_t REDRAW_MS = 500, DEBOUNCE_MS = 30;

// Короткие подписи каналов под 21 символ строки (шрифт GFX — только ASCII)
...
static bool s_btnStable = true, s_btnRead = true;  // pullup: true = отпущена
...
static void redraw(uint32_t now_ms) {
  ...
  switch (s_page) {
    case 0:
      ...
      // без пробела: "MAC" + 17 символов адреса = 20, влезает в 21-символьную строку
      oled.print(F("MAC"));
